...
On Windows, both services run under separate Windows-managed virtual accounts (`NT SERVICE\CSweet.SatelliteOffice.RuntimeHost` and `NT SERVICE\CSweet.SatelliteOffice.Node`) with protected state directories. Only the RuntimeHost virtual account is added to `Hyper-V Administrators`; the Node cannot manage Hyper-V or modify RuntimeHost state. Neither account has write access to the immutable application package. Uninstall removes the RuntimeHost membership. Because Windows grants Hyper-V administrators control over every VM on a host, use a dedicated Satellite Office machine when unrelated or higher-trust Hyper-V workloads are present. Installation is refused on domain controllers.
...
binds the Satellite Office, assignment, workload, provider, canonical specification digest, issue time,
...
Satellite Office reports an explicit `baseline`, `hardened`, or `development` posture. Development
...
$node = Get-Item (Join-Path $payload $manifest.satelliteOfficeExecutable)
if ([Version]$node.VersionInfo.FileVersion -lt [Version]'1.0.2.0') {
    throw "Stale payload: Satellite Office $($node.VersionInfo.FileVersion)"
}

.\scripts\windows\Install-CSweetSatelliteOffice.ps1 `
    -PayloadRoot $payload `
    -ControlPlaneUrl 'https://localhost:54782'
```

Never select an older `certification-*` payload after rebuilding. The installer rejects payloads whose embedded Satellite Office predates reliable signed-assignment delivery.
