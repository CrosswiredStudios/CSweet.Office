...
public sealed record SatelliteOfficeState(
...
public sealed class SatelliteOfficeStateStore(SatelliteOfficeOptions options)
{
    ...
    public async Task<SatelliteOfficeState?> LoadAsync(CancellationToken cancellationToken)
    {
        try
        {
    ...
    public async Task SaveAsync(SatelliteOfficeState state, CancellationToken cancellationToken)
    {
    ...
    public X509Certificate2 GetOrCreateCertificate()
    {
        ...
        var request = new CertificateRequest(
            $"CN=CSweet Satellite Office {Environment.MachineName}", key, HashAlgorithmName.SHA256);
    ...
    public static string CreateCertificateSigningRequestPem(X509Certificate2 certificate)
    {
        using var key = certificate.GetECDsaPrivateKey()
            ?? throw new CryptographicException("The satellite-office identity key is unavailable.");
        var request = new CertificateRequest(
        ...
        string certificateBase64)
    {
        ...
        using (var privateKey = current.GetECDsaPrivateKey()
            ?? throw new CryptographicException("The satellite-office private key is unavailable."))
