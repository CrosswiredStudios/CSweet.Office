...
public sealed class RuntimeHostAuthorizationGate
{
    ...
    public void Pin(A.PinnedHeadquartersTrust requested)
    {
        ...
        lock (_sync)
        {
            ...
            if (existing is not null)
            {
                ...
                if (existing.OfficeId == Guid.Empty && requested.OfficeId != Guid.Empty)
                {
                    WriteAtomic(TrustPath, JsonSerializer.SerializeToUtf8Bytes(requested));
                    return;
                }
                if (existing.OfficeId != requested.OfficeId)
                    throw new InvalidDataException("The pinned Headquarters assignment trust belongs to another Office.");
                return;
            }
            WriteAtomic(TrustPath, JsonSerializer.SerializeToUtf8Bytes(requested));
        }
    ...
    public W.WorkloadSpecification ValidateAndCommit(P.CreateWorkloadRequest request)
    {
        ...
        if (!Guid.TryParse(authorization.OfficeId, out var officeId) || officeId != trust.OfficeId ||
