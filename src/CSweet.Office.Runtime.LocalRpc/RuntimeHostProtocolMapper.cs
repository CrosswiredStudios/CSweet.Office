...
public static class RuntimeHostProtocolMapper
{
    public static W.WorkloadSpecification DeserializeSpecification(string json)
    {
        ...
        if (!document.RootElement.TryGetProperty("kind", out var kind) &&
            !document.RootElement.TryGetProperty("Kind", out kind))
            throw new InvalidDataException("The workload kind is missing.");
        return (W.WorkloadKind)kind.GetInt32() switch
        {
            W.WorkloadKind.Builder => System.Text.Json.JsonSerializer.Deserialize<W.BuilderWorkloadSpecification>(json, options)
                ?? throw new InvalidDataException("The builder workload specification is empty."),
            W.WorkloadKind.Runtime => System.Text.Json.JsonSerializer.Deserialize<W.RuntimeWorkloadSpecification>(json, options)
                ?? throw new InvalidDataException("The runtime workload specification is empty."),
            W.WorkloadKind.ToolchainBuild => System.Text.Json.JsonSerializer.Deserialize<W.ToolchainBuildWorkloadSpecification>(json, options)
                ?? throw new InvalidDataException("The toolchain build workload specification is empty."),
            _ => throw new InvalidDataException("The workload kind is unsupported.")
        };
    }
    ...
    public static P.CreateWorkloadRequest ToProtocol(string providerId, W.WorkloadSpecification workload)
    {
        ...
        switch (workload)
        {
            ...
            case W.RuntimeWorkloadSpecification runtime:
                ...
                break;
            case W.ToolchainBuildWorkloadSpecification toolchain:
                ValidateToolchain(toolchain);
                request.ToolchainBuild = new P.ToolchainBuildSpec
                {
                    ArtifactDigest = toolchain.AdapterArtifact.Digest,
                    ArtifactSignature = toolchain.AdapterArtifact.Signature,
                    ArtifactFormatVersion = toolchain.AdapterArtifact.FormatVersion,
                    ArtifactOperatingSystem = toolchain.AdapterArtifact.OperatingSystem,
                    ArtifactArchitecture = toolchain.AdapterArtifact.Architecture,
                    InstallationId = toolchain.Identity.InstallationId.ToString("D"),
                    BusinessId = toolchain.Identity.BusinessId,
                    TickId = toolchain.Identity.TickId.ToString("D"),
                    RepositoryUrl = toolchain.SourceRepository.RepositoryUrl,
                    CommitSha = toolchain.SourceRepository.CommitSha,
                    IncludeSubmodules = toolchain.SourceRepository.IncludeSubmodules,
                    SourceProfileId = toolchain.SourceRepository.BuildProfileId,
                    SourceProfileVersion = toolchain.SourceRepository.BuildProfileVersion,
                    DeliveryBuildId = toolchain.DeliveryBuildId.ToString("D"),
                    ExpectedBuildRevision = toolchain.ExpectedBuildRevision,
                    RecipeKey = toolchain.RecipeKey,
                    TargetKey = toolchain.TargetKey,
                    ConfigurationJson = toolchain.ConfigurationJson,
                    MaximumSourceBytes = toolchain.MaximumSourceBytes,
                    MaximumOutputBytes = toolchain.MaximumOutputBytes
                };
                request.ToolchainBuild.Entrypoint.Add(toolchain.Entrypoint);
                request.ToolchainBuild.AllowedDependencyRegistryHosts.Add(toolchain.AllowedDependencyRegistryHosts);
                break;
            default:
    ...
    public static W.WorkloadSpecification FromProtocol(P.CreateWorkloadRequest request)
    {
        ...
        {
            W.WorkloadKind.Builder when request.Builder is not null && request.Runtime is null && request.ToolchainBuild is null =>
                Builder(workloadId, image, limits, lease, request.Builder),
            W.WorkloadKind.Runtime when request.Runtime is not null && request.Builder is null && request.ToolchainBuild is null =>
                Runtime(workloadId, image, limits, lease, request.Runtime),
            W.WorkloadKind.ToolchainBuild when request.ToolchainBuild is not null && request.Builder is null && request.Runtime is null =>
                Toolchain(workloadId, image, limits, lease, request.ToolchainBuild),
    ...
    private static W.ToolchainBuildWorkloadSpecification Toolchain(Guid id, W.GuestImageReference image,
        W.WorkloadResourceLimits limits, W.BrokerChannelLease lease, P.ToolchainBuildSpec value)
    {
        var artifact = new W.AgentArtifactReference(value.ArtifactDigest,
            Required(value.ArtifactSignature, nameof(value.ArtifactSignature), 4096),
            Required(value.ArtifactFormatVersion, nameof(value.ArtifactFormatVersion), 50),
            Required(value.ArtifactOperatingSystem, nameof(value.ArtifactOperatingSystem), 50),
            Required(value.ArtifactArchitecture, nameof(value.ArtifactArchitecture), 50));
        if (!Guid.TryParse(value.InstallationId, out var installationId) ||
            !Guid.TryParse(value.TickId, out var tickId) ||
            !Guid.TryParse(value.DeliveryBuildId, out var deliveryBuildId))
            throw new InvalidDataException("The toolchain build identity is invalid.");
        var result = new W.ToolchainBuildWorkloadSpecification(id, image, limits, lease, artifact,
            new W.RuntimeAgentIdentity(installationId, value.BusinessId, tickId),
            new W.RepositoryDescriptor(value.RepositoryUrl, value.CommitSha, value.IncludeSubmodules,
                value.SourceProfileId, value.SourceProfileVersion),
            deliveryBuildId, value.ExpectedBuildRevision, value.RecipeKey, value.TargetKey,
            value.ConfigurationJson, value.Entrypoint.ToArray(),
            value.AllowedDependencyRegistryHosts.ToArray(), value.MaximumSourceBytes, value.MaximumOutputBytes);
        ValidateToolchain(result);
        return result;
    }

    private static void ValidateToolchain(W.ToolchainBuildWorkloadSpecification value)
    {
        ValidateDigest(value.AdapterArtifact.Digest, nameof(value.AdapterArtifact.Digest));
        ValidateRepository(value.SourceRepository);
        if (!string.Equals(value.BrokerLease.ExpectedArtifactDigest, value.AdapterArtifact.Digest, StringComparison.Ordinal) ||
            value.Identity.InstallationId == Guid.Empty || value.Identity.TickId == Guid.Empty ||
            value.DeliveryBuildId == Guid.Empty || value.ExpectedBuildRevision < 1 ||
            value.Entrypoint.Count is < 1 or > 32 || value.Entrypoint.Any(x => string.IsNullOrWhiteSpace(x) || x.Length > 1024) ||
            value.MaximumSourceBytes is < 1 or > 2L * 1024 * 1024 * 1024 ||
            value.MaximumOutputBytes is < 1 or > 16L * 1024 * 1024 * 1024 ||
            value.AllowedDependencyRegistryHosts.Count > 32 ||
            value.AllowedDependencyRegistryHosts.Any(host => string.IsNullOrWhiteSpace(host) || host.Length > 253 ||
                Uri.CheckHostName(host) == UriHostNameType.Unknown))
            throw new InvalidDataException("The toolchain build specification is invalid.");
        _ = Required(value.RecipeKey, nameof(value.RecipeKey), 200);
        _ = Required(value.TargetKey, nameof(value.TargetKey), 200);
        if (value.ConfigurationJson.Length is < 2 or > 256 * 1024)
            throw new InvalidDataException("The toolchain configuration is invalid.");
        using var configuration = System.Text.Json.JsonDocument.Parse(value.ConfigurationJson);
        if (configuration.RootElement.ValueKind != System.Text.Json.JsonValueKind.Object)
            throw new InvalidDataException("The toolchain configuration must be an object.");
    }
