    ...
    ILogger<OfficeWorker> logger) : BackgroundService
{
    ...
    private async Task<OfficeState> EnrollAsync(X509Certificate2 certificate, CancellationToken cancellationToken)
    {
        ...
        var request = new ClaimOfficeRequest(
            ...
            options.AssistedSetupSessionId);
    ...
    internal async Task<X509Certificate2> RefreshOperationalCertificateAsync(
        OfficeState state,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var lifetime = current.NotAfter.ToUniversalTime() - current.NotBefore.ToUniversalTime();
        var renewalLead = TimeSpan.FromTicks(Math.Min(TimeSpan.FromHours(6).Ticks, lifetime.Ticks / 4));
        if (!bootstrapCertificate && current.NotAfter.ToUniversalTime() > DateTime.UtcNow.Add(renewalLead)) return current;
        if (!bootstrapCertificate && current.NotAfter.ToUniversalTime() <= DateTime.UtcNow)
            return await RecoverOperationalCertificateAsync(state, current, cancellationToken);
        ...
        if (response.StatusCode == System.Net.HttpStatusCode.Unauthorized)
            return bootstrapCertificate ? current :
                await RecoverOperationalCertificateAsync(state, current, cancellationToken);
        ...
        if (string.Equals(Normalize(current.Thumbprint), Normalize(issued.CertificateThumbprint ?? string.Empty),
            ...
            return current;
        var installed = stateStore.InstallOperationalCertificate(current, issued.CertificateBase64, issued.CertificateThumbprint);
    ...
    internal async Task<X509Certificate2> RecoverOperationalCertificateAsync(
        OfficeState state, X509Certificate2 current, CancellationToken cancellationToken)
    {
        // Server authentication (including any configured pin) remains mandatory. No expired
        // client certificate or reusable bootstrap receipt is sent to this endpoint.
        using var client = httpClientFactory.CreateClient("control-plane");
        client.Timeout = TimeSpan.FromSeconds(20);
        using var challengeResponse = await client.PostAsync(
            $"api/offices/{state.OfficeId:D}/certificate/challenge", null, cancellationToken);
        ...
        var challenge = await challengeResponse.Content.ReadFromJsonAsync<OfficeCertificateChallengeResponse>(cancellationToken)
            ?? throw new InvalidDataException("The identity recovery challenge was empty.");
        if (challenge.Challenge is null || challenge.Challenge.Length != 44 ||
            challenge.ExpiresAt <= DateTimeOffset.UtcNow)
            throw new InvalidDataException("The identity recovery challenge is invalid or expired.");
        using var key = current.GetECDsaPrivateKey()
            ?? throw new CryptographicException("The enrolled Office private key is unavailable; administrator recovery is required.");
        var signature = key.SignData(OfficeCertificateRecoveryProof.Payload(state.OfficeId, challenge.Challenge),
            HashAlgorithmName.SHA256, DSASignatureFormat.IeeeP1363FixedFieldConcatenation);
        using var response = await client.PostAsJsonAsync($"api/offices/{state.OfficeId:D}/certificate/recover",
            new OfficeCertificateRecoveryRequest(challenge.Challenge, Convert.ToBase64String(signature)), cancellationToken);
        response.EnsureSuccessStatusCode();
        var issued = await response.Content.ReadFromJsonAsync<OfficeCertificateResponse>(cancellationToken)
            ?? throw new InvalidDataException("The identity recovery response was empty.");
        if (!issued.Succeeded || string.IsNullOrWhiteSpace(issued.CertificateBase64) ||
            string.IsNullOrWhiteSpace(issued.CertificateThumbprint))
            throw new InvalidDataException("Headquarters rejected Office identity recovery.");
        var installed = stateStore.InstallOperationalCertificate(current, issued.CertificateBase64, issued.CertificateThumbprint);
        logger.LogInformation("Recovered operational certificate for approved Office {OfficeId}; expires at {ExpiresAt}.",
            state.OfficeId, installed.NotAfter);
        return installed;
    }

    private async Task RenewDuringSessionAsync(OfficeState state, OfficeCertificateLease certificates,
        CancellationToken cancellationToken)
    {
        ...
        while (await timer.WaitForNextTickAsync(cancellationToken))
        {
            try
            {
                ...
                var current = certificates.Current;
                var refreshed = await RefreshOperationalCertificateAsync(state, current, timeout.Token);
                if (!ReferenceEquals(current, refreshed)) certificates.Update(refreshed);
            }
            catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested) { return; }
            catch (Exception exception)
            {
                logger.LogWarning(exception, "Office certificate renewal failed; retrying in one minute.");
            }
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (string.Equals(certificate.Subject, certificate.Issuer, StringComparison.OrdinalIgnoreCase))
        {
            ...
            using var heartbeat = await http.PostAsJsonAsync($"api/offices/{state.OfficeId:D}/heartbeat",
                new OfficeHeartbeatRequest(state.EnrollmentReceipt, state.SessionEpoch,
                    options.AllocatableCpuCount, options.AllocatableMemoryMb, options.AllocatableDiskMb,
                    options.MaximumConcurrentWorkloads, providers, options.SecurityPosture(), typeof(OfficeWorker).Assembly.GetName().Version!.ToString(3)), cancellationToken);
            heartbeat.EnsureSuccessStatusCode();
        }

        using var certificates = new OfficeCertificateLease(certificate);
        using var renewalLifetime = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
        var handler = certificateValidator.CreateRotatingHttpHandler(() => certificates.Current);
        ...
        var renewalTask = RenewDuringSessionAsync(state, certificates, renewalLifetime.Token);
        ...
        try
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                {
                    OfficeVersion = typeof(OfficeWorker).Assembly.GetName().Version!.ToString(3),
        ...
        CancellationTokenSource assignmentCancellation)
    {
        ...
        try
        {
            ...
            var artifactDigest = specification switch
            {
                RuntimeWorkloadSpecification runtime => runtime.Artifact.Digest,
                ToolchainBuildWorkloadSpecification toolchain => toolchain.AdapterArtifact.Digest,
                _ => null
            };
            if (artifactDigest is not null)
            {
                ...
                await artifactCache.EnsureAsync(
                    gatewayClient, state, assignment, artifactDigest, assignmentCancellation.Token);
            }
    ...
    private static WorkloadSpecification DeserializeSpecification(string json)
    {
        ...
        return (WorkloadKind)kind.GetInt32() switch
        {
            WorkloadKind.Builder => JsonSerializer.Deserialize<BuilderWorkloadSpecification>(json, options)
                ?? throw new InvalidDataException("The builder workload specification is empty."),
            WorkloadKind.Runtime => JsonSerializer.Deserialize<RuntimeWorkloadSpecification>(json, options)
                ?? throw new InvalidDataException("The runtime workload specification is empty."),
            WorkloadKind.ToolchainBuild => JsonSerializer.Deserialize<ToolchainBuildWorkloadSpecification>(json, options)
                ?? throw new InvalidDataException("The toolchain workload specification is empty."),
