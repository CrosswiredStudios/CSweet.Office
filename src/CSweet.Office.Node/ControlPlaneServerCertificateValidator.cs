...
public sealed class ControlPlaneServerCertificateValidator
{
    ...
    public ControlPlaneServerCertificateValidator(OfficeOptions options, TimeProvider timeProvider)
    {
    ...
    internal SocketsHttpHandler CreateRotatingHttpHandler(Func<X509Certificate2> currentCertificate) => new()
    {
        ConnectTimeout = TimeSpan.FromSeconds(20),
        SslOptions = new SslClientAuthenticationOptions
        {
            AllowTlsResume = false,
            LocalCertificateSelectionCallback = (_, _, _, _, _) => currentCertificate(),
            RemoteCertificateValidationCallback = (_, certificate, _, errors) =>
                ValidateCertificate(certificate as X509Certificate2, errors)
        }
    };
    ...
    internal bool Validate(
        ...
        X509Chain? __,
        SslPolicyErrors policyErrors) => ValidateCertificate(certificate, policyErrors);

    private bool ValidateCertificate(X509Certificate2? certificate, SslPolicyErrors policyErrors)
    {
