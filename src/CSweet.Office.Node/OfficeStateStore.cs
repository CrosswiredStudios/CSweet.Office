...
public sealed class OfficeStateStore(OfficeOptions options)
{
    ...
    public X509Certificate2 InstallOperationalCertificate(
        ...
        string certificateBase64, string? expectedThumbprint = null)
    {
        ...
        {
            // Validate before replacing the durable identity, so malformed responses cannot
            // corrupt the installed credential. CopyWithPrivateKey also verifies the key binding.
            if (publicCertificate.NotAfter.ToUniversalTime() <= DateTime.UtcNow ||
                publicCertificate.NotBefore.ToUniversalTime() > DateTime.UtcNow ||
                (expectedThumbprint is not null && !string.Equals(publicCertificate.Thumbprint,
                    expectedThumbprint, StringComparison.OrdinalIgnoreCase)))
                throw new CryptographicException("The issued operational certificate is invalid.");
