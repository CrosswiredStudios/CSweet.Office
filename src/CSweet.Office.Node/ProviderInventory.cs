...
{
    public async Task<IReadOnlyList<RegisterOfficeProviderRequest>> ProbeAsync(CancellationToken cancellationToken)
    {
        var inventory = new List<RegisterOfficeProviderRequest>();
        foreach (var provider in providers.Where(IsCurrentPlatformProvider))
        {
            try
            {
                ...
                inventory.Add(new RegisterOfficeProviderRequest(
                    ...
                    SupportsToolchainBuildWorkloads: true,
