...
{
    private readonly string _root = Path.Combine(Path.GetTempPath(), $"csweet-office-{Guid.NewGuid():N}");

    [Fact]
    public void NodeVersionAdvertisesCanonicalAssignmentDigestSupport()
    {
        var version = typeof(OfficeWorker).Assembly.GetName().Version;

        Assert.NotNull(version);
        Assert.True(version >= new Version(0, 1, 0),
            $"Office {version} must not advertise a pre-0.1.0 version without privileged signed-assignment enforcement.");
    }

    [Fact]
    public void DevelopmentPostureRequiresExplicitOfficeConsent()
    {
        var options = new OfficeOptions { SecurityProfile = "development" };
    ...
    public void LocalEndpointUsesBrandedV1Identity()
    {
        ...
        endpoint.Validate();
        Assert.Equal("CSweet:Office:RuntimeHost", RuntimeHostEndpointOptions.SectionName);
        Assert.Equal("csweet-office-runtime-v1", endpoint.NamedPipeName);
        Assert.Contains("csweet-office-runtime-v1.sock", endpoint.UnixSocketPath);
    }
    ...
    public void EnrollmentTokenFileIsDeletedOnlyAfterStateIsSaved()
    {
        var worker = File.ReadAllText(Path.Combine(
            RepositoryRoot(), "src", "CSweet.Office.Node", "OfficeWorker.cs"));
    ...
    public async Task DownloadedArtifactIsClosedBeforeHashVerificationAndCommit()
    {
        ...
        await OfficeArtifactCache.DownloadAndCommitAsync(
