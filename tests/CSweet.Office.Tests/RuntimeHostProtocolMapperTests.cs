...
public sealed class RuntimeHostProtocolMapperTests
{
    ...
    [Fact]
    public void ToolchainRoundTrip_PreservesExactBuildIdentityAndBounds()
    {
        var expected = ToolchainSpec();

        var protocol = RuntimeHostProtocolMapper.ToProtocol("hyperv", expected);
        var actual = Assert.IsType<ToolchainBuildWorkloadSpecification>(RuntimeHostProtocolMapper.FromProtocol(protocol));

        Assert.Equal(expected.WorkloadId, actual.WorkloadId);
        Assert.Equal(expected.DeliveryBuildId, actual.DeliveryBuildId);
        Assert.Equal(expected.ExpectedBuildRevision, actual.ExpectedBuildRevision);
        Assert.Equal(expected.AdapterArtifact, actual.AdapterArtifact);
        Assert.Equal(expected.Identity, actual.Identity);
        Assert.Equal(expected.SourceRepository, actual.SourceRepository);
        Assert.Equal(expected.RecipeKey, actual.RecipeKey);
        Assert.Equal(expected.TargetKey, actual.TargetKey);
        Assert.Equal(expected.ConfigurationJson, actual.ConfigurationJson);
        Assert.Equal(expected.AllowedDependencyRegistryHosts, actual.AllowedDependencyRegistryHosts);
        Assert.Equal(expected.MaximumSourceBytes, actual.MaximumSourceBytes);
        Assert.Equal(expected.MaximumOutputBytes, actual.MaximumOutputBytes);
    }

    [Fact]
    public void ToolchainFromProtocol_RejectsRegistrySchemesAndPorts()
    {
        var protocol = RuntimeHostProtocolMapper.ToProtocol("hyperv", ToolchainSpec());
        protocol.ToolchainBuild.AllowedDependencyRegistryHosts.Clear();
        protocol.ToolchainBuild.AllowedDependencyRegistryHosts.Add("https://registry.npmjs.org:443");
    ...
    private static ToolchainBuildWorkloadSpecification ToolchainSpec()
    {
        var id = Guid.NewGuid();
        return new ToolchainBuildWorkloadSpecification(
            id,
            Image(),
            Limits(),
            Lease(id, ArtifactDigest),
            new AgentArtifactReference(ArtifactDigest, "signature", "1.0", "linux", "x64"),
            new RuntimeAgentIdentity(Guid.NewGuid(), Guid.NewGuid().ToString("D"), Guid.NewGuid()),
            new RepositoryDescriptor("https://github.com/example/game.git", new string('a', 40), false, "phaser.web-2d.v1", "1.0"),
            Guid.NewGuid(),
            7,
            "phaser.web-2d.v1",
            "web",
            "{}",
            ["CSweet.Plugins.Toolchain.NodeTypeScript"],
            ["registry.npmjs.org"],
            100 * 1024 * 1024,
            200 * 1024 * 1024);
    }
