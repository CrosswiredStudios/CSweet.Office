...
public sealed class RuntimeHostRpcIntegrationTests
{
    ...
    public void PrivilegedAuthorizationRejectsTamperingExpiryAndTrustReplacement()
    {
        ...
        Assert.Throws<InvalidDataException>(() => gate.Pin(Trust(replacement, trust.OfficeId)));
    }
    ...
    private static CSweet.Office.Runtime.Abstractions.SignedWorkloadAuthorization Authorization(
        ECDsa key,
        ...
        WorkloadSpecification workload)
    {
        ...
        return new CSweet.Office.Runtime.Abstractions.SignedWorkloadAuthorization(
            ...
            key.SignData(AssignmentEnvelope.Payload(
                trust.OfficeId, assignmentId, workload.WorkloadId, 1,
                providerId, digest, issued, expires), HashAlgorithmName.SHA256),
            issued,
            expires);
    }

    private static CSweet.Office.Runtime.Protocol.SignedWorkloadAuthorization ToProtocol(
        CSweet.Office.Runtime.Abstractions.SignedWorkloadAuthorization authorization) => new()
    {
        AuthorizationVersion = authorization.AuthorizationVersion,
        OfficeId = authorization.OfficeId.ToString("D"),
