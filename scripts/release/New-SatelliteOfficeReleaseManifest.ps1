...
$assets = foreach ($definition in $definitions) {
    foreach ($file in Get-ChildItem -LiteralPath $AssetDirectory -Filter $definition.Pattern -File) {
        ...
        [ordered]@{
            ...
            url = "https://github.com/CrosswiredStudios/CSweet.SatelliteOffice/releases/download/v$Version/$($file.Name)"
...
$manifest = [ordered]@{ schemaVersion = 1; satelliteOfficeVersion = $Version; contractsVersion = $ContractsVersion; protocolVersion = $ProtocolVersion; assets = @($assets) }
