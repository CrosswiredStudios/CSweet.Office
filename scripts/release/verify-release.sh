...
root=$1
for required in satellite-office-release.json SHA256SUMS; do
  [ -s "$root/$required" ] || { echo "Missing release evidence: $required" >&2; exit 2; }
done
(cd "$root" && sha256sum --check SHA256SUMS)
python3 -m json.tool "$root/satellite-office-release.json" >/dev/null
...
for asset in "$root"/*.msi "$root"/*.deb "$root"/*.rpm "$root"/*.pkg; do
  [ -e "$asset" ] || continue
  grep -F "$(basename "$asset")" "$root/satellite-office-release.json" >/dev/null || {
