...
try {
    ...
    & "$env:SystemRoot\System32\sc.exe" start 'CSweet.SatelliteOffice.RuntimeHost' 2>&1 | Out-Host
    Start-Sleep -Seconds 3
} finally {
...
$denied = @(Import-Csv -LiteralPath $csvPath | Where-Object {
    ...
    ($_.ProcessName -in @('services.exe', 'CSweet.SatelliteOffice.RuntimeHost.exe') -or
     $_.Path -match 'CSweet|SatelliteOffice|RuntimeHost')
