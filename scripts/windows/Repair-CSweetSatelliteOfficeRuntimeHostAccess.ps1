...
function Set-ProtectedPackageAcl([string] $Root, [string] $RuntimeHostSid, [string] $NodeSid) {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'The installed Satellite Office package permissions could not be repaired.' }
...
function Stop-OrphanedSatelliteOfficeProcesses([string] $ProtectedInstallRoot) {
    $rootPath = [IO.Path]::GetFullPath($ProtectedInstallRoot).TrimEnd('\') + '\'
    $serviceProcessIds = @(Get-CimInstance -ClassName Win32_Service |
        Where-Object { $_.Name -in @('CSweet.SatelliteOffice.RuntimeHost', 'CSweet.SatelliteOffice.Node') -and [int]$_.ProcessId -gt 0 } |
        ForEach-Object { [int]$_.ProcessId })
    $processNames = @('CSweet.SatelliteOffice.RuntimeHost.exe', 'CSweet.SatelliteOffice.Node.exe')
...
try {
    ...
    if ($null -eq $nodeService) { throw 'The Satellite Office Node Windows service is not installed.' }
    ...
    Stop-OrphanedSatelliteOfficeProcesses -ProtectedInstallRoot $InstallRoot
    ...
    if ($null -eq $configuration.CSweet -or $null -eq $configuration.CSweet.SatelliteOffice -or
        $null -eq $configuration.CSweet.SatelliteOffice.RuntimeHost) {
        throw 'The installed RuntimeHost configuration is invalid.'
    }
    $hyperVConfiguration = $configuration.CSweet.SatelliteOffice.Providers.HyperV
    ...
    $runtimeHostConfiguration = $configuration.CSweet.SatelliteOffice.RuntimeHost
    ...
    if (Test-Path -LiteralPath $nodeDataRoot -PathType Container) {
        ...
        if ($LASTEXITCODE -ne 0) { throw 'The Satellite Office Node state permissions could not be repaired.' }
    }
    ...
    try {
        $stoppedNodeService = Get-Service -Name 'CSweet.SatelliteOffice.Node' -ErrorAction SilentlyContinue
        if ($restartNodeService -and $null -ne $stoppedNodeService -and $stoppedNodeService.Status -ne 'Running') {
            Start-Service -Name 'CSweet.SatelliteOffice.Node'
        }
    } catch { }
    try {
        Write-CSweetSetupProgress -Path $ProgressPath -JobId $ProgressJobId -Workflow 'access-repair' `
