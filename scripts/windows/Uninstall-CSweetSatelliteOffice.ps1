...
function Remove-SatelliteOfficeHyperVResources([string] $DataRoot) {
    ...
    foreach ($vm in $ownedVms) {
        Write-Host "Removing C-Sweet Satellite Office Hyper-V VM '$($vm.Name)'..."
...
Remove-SatelliteOfficeHyperVResources $hyperVDataRoot
...
Write-Host 'C-Sweet Satellite Office was fully uninstalled, including installed versions, cached artifacts, guest disks, and owned Hyper-V VMs. Repository build artifacts were not removed. Revoke the host in fleet administration if it was not already revoked.' -ForegroundColor Green
