...
for required in runtime-manifest.json install-satellite-office.sh uninstall-satellite-office.sh \
  csweet-satellite-office-runtime.service csweet-satellite-office-node.service CSweet.SatelliteOffice.RuntimeHost CSweet.SatelliteOffice.Node; do
  [ -e "$payload_root/$required" ] || { echo "Payload is missing $required." >&2; exit 2; }
done
[ -f "$script_root/configure-satellite-office.sh" ] || {
  echo "configure-satellite-office.sh is missing." >&2; exit 2;
}
...
stage_package_tree() {
  destination=$1
  mkdir -p "$destination/usr/lib/csweet/satellite-office-installer" "$destination/usr/sbin"
  cp -a "$payload_root/." "$destination/usr/lib/csweet/satellite-office-installer/"
  install -m 0755 "$script_root/configure-satellite-office.sh" \
    "$destination/usr/sbin/csweet-configure-satellite-office"
  install -m 0755 "$script_root/uninstall-satellite-office.sh" \
    "$destination/usr/sbin/csweet-uninstall-satellite-office"
}
...
Package: csweet-satellite-office
Version: $version
Section: admin
Priority: optional
...
Depends: systemd
Description: C-Sweet Satellite Office enrollment payload
 Installs the signed, non-secret runtime payload. Run
 csweet-configure-satellite-office separately to enroll this machine.
EOF
  cat > "$deb_root/DEBIAN/postinst" <<'EOF'
#!/bin/sh
set -e
echo "C-Sweet payload installed. Enroll with: sudo csweet-configure-satellite-office https://CONTROL-PLANE"
EOF
  cat > "$deb_root/DEBIAN/prerm" <<'EOF'
#!/bin/sh
set -e
if [ "${1:-}" = remove ] && { [ -e /opt/csweet/satellite-office/CSweet.SatelliteOffice.Node ] || [ -e /etc/systemd/system/csweet-satellite-office-node.service ]; }; then
  /usr/sbin/csweet-uninstall-satellite-office
fi
EOF
  chmod 0755 "$deb_root/DEBIAN/postinst" "$deb_root/DEBIAN/prerm"
  deb_path="$output_root/csweet-satellite-office_${version}_${deb_arch}.deb"
...
build_rpm() {
  ...
  source_name="csweet-satellite-office-$version"
  ...
  cat > "$rpm_root/SPECS/csweet-satellite-office.spec" <<EOF
Name: csweet-satellite-office
Version: $version
Release: 1%{?dist}
Summary: C-Sweet Satellite Office enrollment payload
License: Proprietary
BuildArch: $rpm_arch
...
%preun
if [ \$1 -eq 0 ] && { [ -e /opt/csweet/satellite-office/CSweet.SatelliteOffice.Node ] || [ -e /etc/systemd/system/csweet-satellite-office-node.service ]; }; then
  /usr/sbin/csweet-uninstall-satellite-office
fi

%files
%defattr(-,root,root,-)
/usr/lib/csweet/satellite-office-installer
/usr/sbin/csweet-configure-satellite-office
/usr/sbin/csweet-uninstall-satellite-office
EOF
  rpmbuild --define "_topdir $rpm_root" -bb "$rpm_root/SPECS/csweet-satellite-office.spec"
