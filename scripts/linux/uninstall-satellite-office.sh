...
systemctl disable --now csweet-satellite-office-node.service 2>/dev/null || true
systemctl disable --now csweet-satellite-office-runtime.service 2>/dev/null || true
rm -f -- /etc/systemd/system/csweet-satellite-office-node.service
rm -f -- /etc/systemd/system/csweet-satellite-office-runtime.service
rm -f -- /etc/csweet/satellite-office.env
rm -f -- /etc/csweet/runtime-host.env
systemctl daemon-reload

rm -rf -- /opt/csweet/satellite-office
rm -rf -- /var/lib/csweet/satellite-office
rm -rf -- /var/lib/csweet/satellite-office
...
echo "C-Sweet RuntimeHost and SatelliteOffice were uninstalled. Revoke the node in fleet administration if it was not already revoked."
